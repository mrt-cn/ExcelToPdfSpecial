...
class PDFConverterApp:
    ...
    def normalize_date(self, value):
        """ Tarih formatını normalize eder ve sadece tarihi alır (d.M.yyyy formatında) """
        if isinstance(value, str):  # Eğer değer bir string ise
            try:
                # Eğer tarih ve saat bir aradaysa (örneğin, '11.2.2024 12:34:56')
                if " " in value:
                    value = value.split(" ")[0]  # Sadece tarih kısmını al

                # Eğer "/" içeriyorsa, formatı kontrol et
                if "/" in value:
                    try:
                        # İngiliz formatı (mm/dd/yyyy)
                ...
                # Eğer "-" içeriyorsa, ISO formatı (yyyy-mm-dd)
                elif "-" in value:
                    return datetime.strptime(value, "%Y-%m-%d").strftime("%-d.%-m.%Y")

            except ValueError:
                # Eğer format tanımlanamazsa, olduğu gibi döndür
                print(f"Geçersiz tarih formatı: {value}")
        return value  # Eğer string değilse veya tanımlanamıyorsa, olduğu gibi döndür
